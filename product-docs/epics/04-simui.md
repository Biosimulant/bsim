...
- Run parameter overrides need a clean interface that does not mutate private state.

## Acceptance criteria

- SimUI is optional:
  - Base `import bsim` works with no UI deps.
  - `import bsim.simui` errors with a clear message when `.[ui]` extras are not installed.
...
- Run overrides use an explicit interface (or are passed as run params for module consumption).
...
3. Add explicit run parameter handling:
   - pass run params through the API, and
   - document how modules can consume those params.
